...
# Ejemplo de Algoritmo Genético para Selección de Características
import numpy as np
...
TAMANO_POBLACION = 50
LONGITUD_CROMOSOMA = X.shape[1]  # El número de características
GENERACIONES = 100
TASA_MUTACION = 0.05


def crear_individuo():
    """Crea un individuo (cromosoma) aleatorio, una lista de 0s y 1s."""
    return np.random.randint(2, size=LONGITUD_CROMOSOMA)


def calcular_aptitud(individuo):
    """Calcula la aptitud (precisión) de un individuo."""
    # Obtenemos los índices de las características seleccionadas (donde el valor es 1)
    ...
    # Creamos subconjuntos de datos con solo las características seleccionadas
    ...
    # Hacemos predicciones y calculamos la precisión (aptitud)
...
def seleccionar_padres(poblacion, puntajes_aptitud):
    """Selecciona dos padres usando la selección de ruleta."""
    total_aptitud = sum(puntajes_aptitud)

    # Evita la división por cero si todas las aptitudes son 0
    if total_aptitud == 0:
        total_aptitud = 1

    probabilidades = [puntaje / total_aptitud for puntaje in puntajes_aptitud]

    # Elegimos dos padres aleatoriamente, con mayor probabilidad para los más aptos
...
def mutar(individuo):
    ...
    for i in range(LONGITUD_CROMOSOMA):
        if np.random.rand() < TASA_MUTACION:
            individuo[i] = 1 - individuo[i]  # Invierte el bit (0 a 1, o 1 a 0)
    return individuo
...
for generacion in range(GENERACIONES):
    ...
    # Elitismo: mantenemos al mejor individuo de la generación
